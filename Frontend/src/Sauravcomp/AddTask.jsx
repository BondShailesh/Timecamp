...
const AddTask = ({ isAdd ,note,startduration,endduration,setAdd,settask,task,handlePost}) => {
      ...
      <div className="flex-col  w-full justify-evenly items-center  ">
          ...
          <div className="flex justify-around mt-2 mb-3">
            {isnewProject ? (
              ...
              <input
                className=' w-full !outline-none' onChange={(e)=>settask(e.target.value)} type={"text"}
                placeholder="Add new Project Name"
              />
              <button className='font-bold' onClick={()=>setisnewProject(false)}>x</button>
              </div>
            ) : (
              <>
          ...
          <div>
            ...
            <input  className=' w-full !outline-none  mb-4'  type="text" onChange={(e)=>settask(e.target.value)}   name="task_name" placeholder=" Add New Task Name" />
          </div>

          <div className='w-full  gap-4 flex flex-row-reverse'>
            <button className="bg-green-600 m-3 text-slate-100 p-2 rounded-lg"  onClick={()=>{handlePost(task);
            setAdd(false)}
